...
export default class GameController {
  ...
  async onCellClick(index) {
    ...
    if (!this.blockedBoard) {
      if (this.gamePlay.boardEl.style.cursor === 'not-allowed') {
        ...
        // если персонаж соответствует списку пользовательских
  ...
  async characterAttacking(attacker, target) {
    ...
    let damage = Math.floor(Math.max(attacker.attack - target.character.defence, attacker.attack * 0.1));
  ...
  nextLevel() {
    ...
    // добавляем стартовые позиции врагам
...
// считаем максимальное количество очков
  ...
  //Получаем все занятые позиции персонажей
  getFilledPositions() {
    let filledPositions = [];
    for (let i = 0; i < this.userPositions.length; i++) {
    ...
    if (this.currentMove === 'enemy') {
      ...
      for (let itemUser of [...this.userPositions]) {
        ...
        if (allowedEnemyDistanceAttack.includes(itemUser.position)) {
          ...
          let attackPromise = this.characterAttacking(randomEnemy.character, itemUser);
        } else {
          // если нет-переставляем врага на новую случайную позицию
  ...
  getEnemyAttack(user, enemy) {
    if (this.currentMove === 'enemy') {
      //после атаки персонажа создаем массив возможных атак врага на которого напали
      ...
      if(allowedEnemyDistanceAttack.includes(user.position) && enemy.character.health > 0) {
        ...
        enemy.position = this.getRandom(this.allowedPositions(enemy.position, enemy.character.distance));
        this.gamePlay.redrawPositions([...this.userPositions, ...this.enemyPositions]);
        this.currentMove = 'user';
      }
    }
